...
/**
 * An applet that demonstrates the graph algorithm, by letting the user pick the
 * points in the screen and choose either the Quick Hull algor or Brute Force
 * algorithm, the program simulated the execution event, showing which line or
 * which point is being compared.
 *
 * @author Jeff C. So
 * @version %I%, %G%
 */
...
class SrcPanel extends Panel implements Runnable {
    ...
    /**
     * Display all the points and line in the hull. If we are in execution mode,
     * also display all the lines and points we are checking
     */
            ...
            if (runMode) {
                switch (algor) {
                    ...
                    case MENU:
                        ...
                        break;
                    default:
                        System.out.println("Error in call algor\n");
                }
    ...
    /**
     * Quick Hull Algorithm implementation. Calculate the hull first and display
     * the execution with the information from chklns and tempHull.
     */
